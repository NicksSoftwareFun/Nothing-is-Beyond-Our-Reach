...
It launches under ALARM or by scenario, and it hunts by the Warden's law: it attacks only a **Known** Crew figure or vehicle ({{ref:sec-known}}). Each Warden Phase aloft it moves up to {{stat:drones.eca-fiber-fpv.move}} squares toward its target; when it can strike, the Warden makes its Pilot test at **11**. Success: detonation on the target's square, immediately — blast attack 12/14 at DR {{stat:weapons.fpv.dr}}, Pen {{stat:weapons.fpv.pen}}, dives allowed, exactly like your own FPV in enemy hands. Failure: drift d8 × d6 halved and detonate. Hit or drift, **the airframe is expended** — every fiber FPV is one strike.
