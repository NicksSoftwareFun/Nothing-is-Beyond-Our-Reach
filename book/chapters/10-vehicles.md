...
**Warden Phase.** The technical's element acts by Doctrine: the gunner, stationary, puts an Aimed burst after the truck. TN 8, +3 fast mover — **11**. The Warden rolls **7 and 5 — 12.** Struck. Damage Die: d10 = **2**, +3 Pen, −0 — **5: Mobility hit.** Rear tires gone to rag: both Speeds halve, rounding halves up — road {{stat:vehicles.box-truck.speed_road}} to 2, rough already 1. The truck limps — 12 squares a turn on rims instead of 18, still six people carried faster than any of them can run, and Delgado's Mechanic skill waits at the first quiet culvert: an Interact, 4 AP, Tool Roll, once per vehicle, and the tires are cut brush and prayer but the Speed comes back.
